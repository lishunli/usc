	...
	private static void test_list_t() throws Exception {
		...
		if (oauth.getStatus() == 1) {
			...
			if (oauth.getStatus() == 2) {
				...
				/*
				 * Ht_API htAPI=new Ht_API(); String response=htAPI.info(oauth,WeiBoConst.ResultType.ResultType_Json,"20");
				 */
				...
				/*
				 * Statuses_API statuses_API=new Statuses_API(); String response=statuses_API.ht_timeline(oauth,
				 * WeiBoConst.ResultType.ResultType_Json,"pBroad","1","","100" );
				 */
				/*
				 * T_API htAPI=new T_API();
				 * url="http://box.zhangmen.baidu.com/m?rf=idx&ct=134217728&tn=baidumt&gate=10&c_n=mp3ordeqqr&l_id=3&l_n=%E6%AD%8C%E6%9B%B2TOP500&s_o=0"; String
				 * response=htAPI.add_music(oauth, WeiBoConst.ResultType.ResultType_Json, "hello", "127.0.0.1", "", "", url, "test", "test");
				 */
				/*
				...
				// String response = "更新用户教育信息:" + tUserAPI.update_edu(oauth, WeiBoConst.ResultType.ResultType_Json, feildid, yearUser, schoolid, departmentid,
				// levelnum) + "\n\r";
				String pagetime = "0";
				String reqnum = "20";
				String lastid = "0";
				...
				// String response = "用户提及时间线索引:" + tStatAPI.mentions_timeline_ids(oauth, WeiBoConst.ResultType.ResultType_Json, pagetime, reqnum, lastid, type,
