...
public class BlacklistDAO extends HibernateDaoSupport
{
	...
	public Blacklist findById(java.lang.Integer id)
	{
		...
		try
		{
			Blacklist instance = (Blacklist) getHibernateTemplate().get("com.usc.daos.Blacklist", id);
			return instance;
		}
		catch (RuntimeException re)
		{
	...
	{
		log.debug("finding Blacklist instance with property: " + propertyName + ", value: " + value);
		try
		{
			String queryString = "from Blacklist as model where model." + propertyName + "= ?";
	...
	public Blacklist merge(Blacklist detachedInstance)
	{
		...
		try
		{
			Blacklist result = (Blacklist) getHibernateTemplate().merge(detachedInstance);
