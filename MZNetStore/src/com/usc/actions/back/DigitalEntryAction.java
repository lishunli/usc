...
public class DigitalEntryAction extends ActionSupport
{
	...
	/**
	 * 下面的验证 productTypeID //不能为0，如果为0提示请选择 digitalName //不能为空 brand //品牌不能为空
	 * barcode 不能为空 且 固定格式 而且唯一 publishedPrice//定价 不能为空且为float类型（类型判断）
	 * productsPflag//初始时设置为0 upload//图片，上传有一定的大小和格式限制（可以为空）这个验证在Struts的配置文件中
	 * 
	 */
	...
	public void validate()
	{
		...
		{
			if (null == digital.getBarcode() || "".equals(digital.getBarcode().trim()))// 条形码
																						// 不能为空
																						// 而且唯一
																						// 且
																						// 固定格式
			...
			else if (manger.checkBarCodeIsExist(digital.getBarcode().trim()))// 条形码
																				// 不能为空
																				// 而且唯一
																				// 且
																				// 固定格式
			...
			if (null == digital.getBrand() || "".equals(digital.getBrand().trim()))// brand
																					// 品牌不能为空
			{
	...
	public String execute() throws Exception
	{
		if (null != digital)// 传过来的值不为空
		{
			if (null != upload)
			{
				FileInputStream fin = new FileInputStream(upload);// File 转
																	// InputStream
