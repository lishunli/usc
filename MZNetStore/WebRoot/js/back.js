...
onload = setUp; // 加载运行的方法
onloadFlag = true;//是否加载
...
function processResponse()
{
	if (XMLHttpReq.readyState == 4)
	{
		if (XMLHttpReq.status == 200)
		{
				...
				if(onloadFlag && types.length == 0)
				{
					alert("加载数据失败，请重新初始化数据库！");
				}
				else
				{	
				...
				{// 第一级下拉框选择0项（也即请选择选项）时，后面下拉框全部清空
					sType.options.length = 0;
					tType.options.length = 0;
					frType.options.length = 0;
				}
				else if (no == 1 && fType.value != 0)
				{// 第一级下拉框选择不为0项（也即选择请选择的其他选项）时，后面下拉框全部清空后再更新数据
					sType.options.length = 0;
					tType.options.length = 0;
					frType.options.length = 0;
				...
				{// 第二级下拉框选择0项（也即请选择选项）时，后面下拉框全部清空
					tType.options.length = 0;
					frType.options.length = 0;
				}
				else if (no == 2 && sType.value != 0)
				{// 第二级下拉框选择不为0项（也即选择请选择的其他选项）时，后面下拉框全部清空后再更新数据
					tType.options.length = 0;
					frType.options.length = 0;
					addOption(tType, types);
				}
				else if (no == 3 && tType.value == 0)
				{// 第三级下拉框选择0项（也即请选择选项）时，后面下拉框全部清空
					frType.options.length = 0;
				}
				else if (no == 3 && tType.value != 0)
				{// 第三级下拉框选择不为0项（也即选择请选择的其他选项）时，后面下拉框全部清空后再更新数据
					frType.options.length = 0;
 ...
 * sType.options.length = 0; tType.options.length = 0; frType.options.length =
 * 0; } else if ((no == 1 && fType.value != 0) || sType.value == "") {//
 * 第一级下拉框选择不为0项（也即选择请选择的其他选项）时，后面下拉框全部清空后再更新数据 sType.options.length = 0;
 * tType.options.length = 0; frType.options.length = 0; addOption(sType, types); }
 */
