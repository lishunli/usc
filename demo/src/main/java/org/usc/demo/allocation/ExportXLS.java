...
//        Sheet sheet2 = wb.createSheet("HSSF_Sheet_2");
        ...
        for (int i = 0; i < 60; i = i + 2) {
            ...
            for (int j = 0; j < 25; j++) {
                ...
                // 创建一个基本的样式
                CellStyle cellStyle = ExportXLS.createStyleCell(wb);
                // 获得这一行的每j列
                cell = row.createCell(j);
                if (j == 0) {
                    // 设置文字在单元格里面的位置
                    cellStyle = ExportXLS.setCellStyleAlignment(cellStyle, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER);
                    ...
                    cellStyle = ExportXLS.setCellStyleAlignment(cellStyle, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER);
                    // 设置这个样式的格式(Format)
                    cellStyle = ExportXLS.setCellFormat(helper, cellStyle, "#,##0.0000");
                    // 先创建字体样式,并把这个样式加到单元格的字体里面
                    cellStyle.setFont(createFonts(wb));
                    // 把这个样式加到单元格里面
                    cell.setCellStyle(cellStyle);
                    ...
                    cellStyle = ExportXLS.setCellStyleAlignment(cellStyle, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER);
                    cellStyle = ExportXLS.setCellFormat(helper, cellStyle, "MM-yyyy-dd");
                    cell.setCellStyle(cellStyle);
                    ...
                    cellStyle = ExportXLS.setCellStyleAlignment(cellStyle, CellStyle.ALIGN_CENTER, CellStyle.VERTICAL_CENTER);
                    cellStyle = ExportXLS.setFillBackgroundColors(cellStyle, IndexedColors.ORANGE.getIndex(), IndexedColors.ORANGE.getIndex(), CellStyle.SOLID_FOREGROUND);
                    cell.setCellStyle(cellStyle);
                    cell.setCellValue(1);
                }
            }
