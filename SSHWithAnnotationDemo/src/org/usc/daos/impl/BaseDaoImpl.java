...
import org.usc.daos.MyHibernateDaoSupport;

/**
 * Dao层接口的实现类 许多人习惯根据不多的业务逻辑定义不同的DAO层接口，如UserDao,NewsDao,CompanyDao等等， 这样往往使得编码量十分庞大，而且带来了维护的困难，因此，抽取此DAO层接口，收录大部分 DAO层必须的方法，以供Service层调用。
 * 
 * @author rongxinhua
 * @version 1.0
 * @param <T>
...
public class BaseDaoImpl<T, PK extends Serializable> extends MyHibernateDaoSupport // implements EntityDao<T, PK>{
{
	/**
	 * 保存实体 包括添加和修改
	 * 
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<T> findByCriteria(final Class<T> entityClass, final String propertyName, final boolean isAsc, final int firstResult, final int maxResults,
	 ...
	 */
	/*
	 * @SuppressWarnings("unchecked") public Pager<T> findForPager(final Class<T> entityClass,final String propertyName,final boolean isAsc,final int
	 * firstResult,final int maxResults,final Criterion criterions){ int totalCounts = findCountsByCriteria(entityClass, criterions); List<T> entityList =
	 * findByCriteria(entityClass, propertyName, isAsc, firstResult, maxResults, criterions); Pager pager = new Pager(); pager.setTotalCounts(totalCounts);
	 * pager.setEntityList(entityList); return pager; }
	 */
...
/*	public List<T> findByProperty(Class<T> entityClass, String propertyName, Object[] value)
	 ...
	 */
	/*
	 * @SuppressWarnings("unchecked") public Pager<T> findForPagerByHql(final int firstResult, final int maxResults, final String hql, final Object params){
	 * Pager<T> pager = (Pager<T>)getHibernateTemplate().execute(new HibernateCallback(){
	 * 
	 * public Object doInHibernate(Session session) throws HibernateException, SQLException { Query query = session.createQuery(hql); for(int position = 0;
	 * position < params.length; position ++){ query.setParameter(position, params[position]); } int totalCounts = query.list().size(); //总记录数 //用于分页查询
	 * if(maxResults > 0){ query.setFirstResult(firstResult); query.setMaxResults(maxResults); } List<T> list = query.list(); Pager<T> pager = new Pager<T>();
	 * pager.setEntityList(list); pager.setTotalCounts(totalCounts); return pager; } }); return pager; }
	 */
