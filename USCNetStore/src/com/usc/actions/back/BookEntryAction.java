...
public class BookEntryAction extends ActionSupport
{
	...
	/**
	 * 下面的验证 productTypeID //不能为0，如果为0提示请选择 bookName //不能为空 publisher //不能为空
	 * isbn //不能为空 且 固定格式 而且唯一 publishedPrice//不能为空且为float类型（类型判断）
	 * publishedTime//可以为空，但不为空时必须满足日期格式 productsPflag//初始时设置为0
	...
	public void validate()
	{
		...
		{
			if (null == book.getIsbn() || "".equals(book.getIsbn().trim()))// isbn
																			// 不能为空
																			// 而且唯一
																			// 且
																			// 固定格式
			...
			else if (manger.checkIsbnIsExist(book.getIsbn().trim()))// isbn 不能为空
																	// 而且唯一 且
																	// 固定格式
			{
