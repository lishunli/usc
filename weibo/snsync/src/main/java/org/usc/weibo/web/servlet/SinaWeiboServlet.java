...
public class SinaWeiboServlet extends BaseServlet {
	...
	private final static String URL_PREFIX = Constants.WEIBO_URL_PREFIX + "sinaweibo";
	...
	public void auth(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!"SINA".equals(provider)) {
				log.info("not sina weibo  please check, " + appId);
				return;
			}
			...
			// System.out.println(requestToken.getAuthorizationURL(URL_PREFIX + "?action=callBack&appId=" + appId));

			// safely read local var.
			// request.setAttribute(APP_ID, appId);
			// request.setAttribute(WEIBO, weibo);
			// request.setAttribute(REQUEST_TOKEN, requestToken);

			// request.getRequestDispatcher(URL_PREFIX + "?action=callBack").forward(request, response);
			response.sendRedirect(requestToken.getAuthorizationURL(URL_PREFIX + "?action=callBack"));
	...
	public void callBack(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			// RequestToken requestToken = (RequestToken) request.getAttribute(REQUEST_TOKEN);
	...
	private String getAppId(HttpServletRequest request) {
		...
		if (StringUtils.isBlank(appId)) {
			Application app = appService.findAppByProvider(Provider.SINA);
