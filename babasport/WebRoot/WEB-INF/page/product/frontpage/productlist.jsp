    ...
    <div class="browse_left">
         <div class="browse">
            <div class="browse_t">${producttype.name}</div>
				...
				<li class='bj_blue'><a href="<html:rewrite action="/product/list"/>?typeid=${childtype.typeid}">${childtype.name}</a></li></c:forEach>		    
			</ul>
	     </div>
<DIV id="sy_biankuang">
    ...
    <div class="browse_right">
         <div class="select_reorder">
              ...
              <a title='按销量降序' href="<html:rewrite action="/product/list"/>?sort=selldesc&typeid=${param.typeid}&style=${param.style}">销量多到少</a></c:if>
			  | <c:if test="${'sellpricedesc'==param.sort}"><strong><em>价格高到低</em></strong></c:if><c:if test="${'sellpricedesc'!=param.sort}">
			  <a title='价格高到低' href="<html:rewrite action="/product/list"/>?sort=sellpricedesc&typeid=${param.typeid}&style=${param.style}">价格高到低</a></c:if>
			  | <c:if test="${'sellpriceasc'==param.sort}"><strong><em>价格低到低</em></strong></c:if><c:if test="${'sellpriceasc'!=param.sort}">
			  <a title='价格低到高' href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&typeid=${param.typeid}&style=${param.style}">价格低到高</a></c:if>
			  | <c:if test="${empty param.sort}"><strong><em>最近上架时间</em></strong></c:if><c:if test="${!empty param.sort}">
			  <a title='价格低到高' href="<html:rewrite action="/product/list"/>?sort=&typeid=${param.typeid}&style=${param.style}">最近上架时间</a></c:if> </div>
              
		      <div class="reorder_r">显示方式：<c:if test="${param.style=='imagetext'}"><strong><em>图文版</em></strong></c:if><c:if test="${param.style!='imagetext'}">
		      <a href="<html:rewrite action="/product/list"/>?sort=${param.sort}&typeid=${param.typeid}&sex=${param.sex }&brandid=${param.brandid}&style=imagetext">图文版</a></c:if> |
		      <c:if test="${param.style=='imagetext'}"><a href="<html:rewrite action="/product/list"/>?sort=${param.sort}&typeid=${param.typeid}&sex=${param.sex }&brandid=${param.brandid}&style=image">图片版</a>
			...
			<div class="emptybox"></div>
			 <div class="brand">
				...
				<li><a href="<html:rewrite action="/product/list"/>?sort=${param.sort}&typeid=${param.typeid}&brandid=${brand.code}&sex=${param.sex}">${brand.name}</a></li></c:forEach>
				</ul>
			 </div>
			 <div class="SubCategoryBox">
				<div class="FindByHint">按<strong>男女款</strong>选择：</div>
				<ul class="CategoryListTableLevel1">
				<li><a  href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&typeid=${param.typeid}&sex=MAN&brandid=${param.brandid}&style=${param.style}">男款</a></li>
				<li><a  href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&typeid=${param.typeid}&sex=WOMEN&brandid=${param.brandid}&style=${param.style}">女款</a></li>
				<li><a  href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&typeid=${param.typeid}&sex=NONE&brandid=${param.brandid}&style=${param.style}">男女均可</a></li>
				<li><a class="red" href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&typeid=${param.typeid}">全部</a></li>
				</ul>
			 </div>
		</div>
...
<c:forEach items="${pageView.records}" var="entry">	
        <div class="detail">
           ...
           <div class="an_img" align="center"><a href="/html/product/${entry.type.typeid }/${entry.id}.html"><img src='/images/sale.gif' width='84' height='24' border='0' class='a_1' /></a></div>
        </div>
</c:forEach>
<!----------------------LOOP END------------------------------->
		<div class='emptybox'></div>
	     ...
	     <c:forEach begin="${pageView.pageindex.startindex}" end="${pageView.pageindex.endindex}" var="wp">
		    ...
		    <c:if test="${pageView.currentpage!=wp}"><div class="page"><a href="<html:rewrite action="/product/list"/>?page=${wp}&brandid=${param.brandid}&sex=${param.sex}&sort=selldesc&style=${param.style}&typeid=${param.typeid}">[${wp}]</a></div></c:if>
		</c:forEach>
		<div>&nbsp;&nbsp;</div>跳转到第
		...
		function topage(pagenum){
			window.location.href='<html:rewrite action="/product/list"/>?sort=selldesc&style=${param.style}&brandid=${param.brandid}&sex=${param.sex}&typeid=${param.typeid}&page='+ pagenum;
		}
		//-->
		</SCRIPT>
         </div>
